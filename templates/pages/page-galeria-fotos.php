            ...
            <ul class="archivo-galeria__lista lista" data-view="<?php echo esc_attr($vista); ?>">
                ...
                <?php if ($query->have_posts()) : ?>
                    <?php while ($query->have_posts()) :
                        ...
                        $galerias  = get_field('fotos');
                        $titulo    = get_the_title();
                        $firtimage = $galerias ? $galerias[0] : [];
                    ?>
                    <li class="lista__item item" aria-labelledby="titulo-<?php echo get_the_ID(); ?>">
                        <a class="item__card" href="<?php echo esc_url(get_permalink()); ?>">
                            <img class="item__imagen" src="<?php echo esc_url($firtimage['url']); ?>" alt="<?php echo esc_attr($firtimage['alt']); ?>">
                            <h2 class="item__titulo body-1 body-bold text-pi" id="titulo-<?php echo get_the_ID(); ?>"><?php echo esc_html($titulo); ?></h2>
                        </a>
                    </li>
                    <?php endwhile; ?>
                <?php endif; ?>
